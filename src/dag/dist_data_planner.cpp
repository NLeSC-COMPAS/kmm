...
template<size_t N>
BufferRequirement DistDataPlanner<N>::add_chunk(
    TaskGraph& graph,
    MemoryId memory_id,
...
template<size_t N>
std::pair<DataDistribution<N>, std::vector<BufferId>> DistDataPlanner<N>::finalize(TaskGraph& graph
) {
    auto dist = DataDistribution<N>::from_chunks(m_shape, std::move(m_chunks), m_buffers);
    return {std::move(dist), std::move(m_buffers)};
}

KMM_INSTANTIATE_ARRAY_IMPL(DistDataPlanner)
